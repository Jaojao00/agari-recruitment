...
layout = layout.replace(
  /import { Toaster } from "@\/components\/ui\/toaster";/g,
  "",
);
...
validation = validation.replace(
  /z\.enum\(\['Nam', 'N.?'\]/g,
  "z.enum(['Nam', 'Nữ'] as [string, ...string[]]",
);
validation = validation.replace(
  /z\.enum\(\['9\/12', '10\/12', '11\/12', '12\/12', 'Khác'\]/g,
  "z.enum(['9/12', '10/12', '11/12', '12/12', 'Khác'] as [string, ...string[]]",
);
validation = validation.replace(
  /z\.enum\(\['Ca 1: 06:00 - 15:00', 'Ca 2: 15:00 - 22:00', 'Ca 3: 18:00 - 22:00'\]/g,
...
let adminApiRoute = fs.readFileSync(
...
let adminApiDetail = fs.readFileSync(
  "src/app/api/admin/applications/[id]/route.ts",
  "utf8",
);
adminApiDetail = adminApiDetail.replace(
  /async function GET\(request: Request, { params }: { params: { id: string } }\)/g,
  "async function GET(request: Request, { params }: any)",
);
adminApiDetail = adminApiDetail.replace(
  /async function PATCH\(request: Request, { params }: { params: { id: string } }\)/g,
  "async function PATCH(request: Request, { params }: any)",
);
fs.writeFileSync(
  "src/app/api/admin/applications/[id]/route.ts",
  adminApiDetail,
...
let adminExportApi = fs.readFileSync(
...
firebaseAdmin = firebaseAdmin.replace(
  /admin\.credential/g,
...
let adminAppPage = fs.readFileSync(
  "src/app/admin/applications/page.tsx",
  "utf8",
);
adminAppPage = adminAppPage.replace(
  /setStatusFilter\(val\)/g,
...
adminDetail = adminDetail.replace(
  /onValueChange={setStatus}/g,
