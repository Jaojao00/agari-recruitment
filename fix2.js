...
adminReportsPage = adminReportsPage.replace(/setStatus\(val\)/g, 'setStatus(val || "")');
fs.writeFileSync('src/app/admin/reports/page.tsx', adminReportsPage);

let apiAppsRoute = fs.readFileSync('src/app/api/admin/applications/route.ts', 'utf8');
apiAppsRoute = apiAppsRoute.replace(/doc =>/g, '(doc: any) =>');
fs.writeFileSync('src/app/api/admin/applications/route.ts', apiAppsRoute);

let apiExportRoute = fs.readFileSync('src/app/api/admin/export/excel/route.ts', 'utf8');
