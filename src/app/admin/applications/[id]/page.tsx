  ...
  const fetchApplication = async () => {
    try {
      ...
      if (res.ok) {
        ...
        if (app.hiredAt) setHiredAt(new Date(app.hiredAt._seconds * 1000).toISOString().split('T')[0]);
        if (app.expiredAt) setExpiredAt(new Date(app.expiredAt._seconds * 1000).toISOString().split('T')[0]);
      }
    } catch (error) {
      console.error(error);
    } finally {
  ...
  return (
    <div className="space-y-6 max-w-4xl mx-auto pb-12">
      <div className="flex items-center gap-4">
        ...
        <h1 className="text-2xl font-bold">Chi tiết hồ sơ: <span className="text-red-700">{data.applicationId}</span></h1>
      </div>
      ...
      <div className="grid md:grid-cols-2 gap-6">
        <Card>
          ...
          <CardContent className="space-y-4">
            ...
            <div className="grid grid-cols-3 gap-2 py-2 border-b">
              ...
              <span className="col-span-2 italic text-gray-600">{data.note || 'Không có'}</span>
            </div>
          </CardContent>
        </Card>
        ...
        <div className="space-y-6">
          ...
          <Card>
            <CardHeader>
              <CardTitle className="text-sm">Trạng thái đồng bộ Google Sheets</CardTitle>
            </CardHeader>
            <CardContent>
              <div className="flex items-center justify-between">
                <div>
                  <span className={`px-2 py-1 text-xs rounded-full font-bold
                    ${data.googleSheetSyncStatus === 'SUCCESS' ? 'bg-green-100 text-green-700' : 
                      data.googleSheetSyncStatus === 'FAILED' ? 'bg-red-100 text-red-700' : 'bg-yellow-100 text-yellow-700'}
                  `}>
                    {data.googleSheetSyncStatus}
                  </span>
                  {data.googleSheetSyncError && <p className="text-xs text-red-500 mt-2">{data.googleSheetSyncError}</p>}
                </div>
                <Button variant="outline" size="sm" onClick={() => {
                   fetch('/api/sync/process', { method: 'POST' }).then(() => fetchApplication());
                }}>
