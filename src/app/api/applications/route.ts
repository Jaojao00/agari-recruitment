...
export async function POST(request: Request) {
  try {
    ...
    // Checking CCCD
    const cccdQuery = await applicationsRef.where('cccd', '==', validatedData.cccd).get();
    if (!cccdQuery.empty) {
    ...
    // Checking Phone
    const phoneQuery = await applicationsRef.where('phone', '==', validatedData.phone).get();
    if (!phoneQuery.empty) {
    ...
    const newAppId = await adminDb.runTransaction(async (transaction) => {
      ...
      transaction.set(counterRef, { applicationCount: newCount }, { merge: true });
    ...
    fetch(`${baseUrl}/api/sync/process`, { method: 'POST' }).catch(e => console.error('Background sync failed to start:', e));

    return NextResponse.json({ success: true, applicationId: newAppId }, { status: 201 });
